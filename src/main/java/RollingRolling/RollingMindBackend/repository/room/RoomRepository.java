...
public interface RoomRepository extends JpaRepository<Room, Long> {
    ...
    @Query(value = "SELECT r.idx AS idx, u.nickname AS nickname, r.room_id AS roomId, r.open AS open, r.participation_request AS participationRequest, r.title AS title, r.content AS content, r.release_date AS releaseDate, r.template_type AS templateType, COUNT(p.room_id) AS participantsCount " +
        "FROM rooms r " +
        "LEFT JOIN participants p ON r.room_id = p.room_id " +
        "LEFT JOIN users u ON r.host_id = u.id " +
        "GROUP BY r.idx, u.nickname, r.room_id, r.open, r.participation_request, r.title, r.content, r.release_date, r.template_type " +
